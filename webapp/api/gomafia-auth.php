...
if ($action === 'login') {
    ...
    try {
        ...
        if (!$csrfToken) {
            echo json_encode(['success' => false, 'error' => 'Не удалось получить CSRF токен']);
            cleanup($cookieFile);
            exit();
        }
...
function getCSRFToken($cookieFile) {
    ...
    // Fallback: try to extract token from login page HTML
    ...
    if ($html) {
        ...
        if (preg_match('/__NEXT_DATA__[^>]*>(.*?)<\/script>/', $html, $m)) {
