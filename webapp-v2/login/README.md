...
```bash
mysql -u USERNAME -p DATABASE_NAME < webapp-v2/login/auth-migration.sql
```
...
Откройте `webapp-v2/login/auth-config.php` и укажите:
...
Откройте `webapp-v2/login/bot.js` и укажите:

```javascript
const BOT_TOKEN = 'YOUR_BOT_TOKEN_HERE';   // Тот же токен
const CONFIRM_API_URL = 'https://your-domain.com/login/code-confirm.php';
```
...
export CONFIRM_API_URL="https://your-domain.com/login/code-confirm.php"
```
...
Пользователь открывает приложение в браузере
  → AuthGate запрашивает POST /login/code-generate.php
  ...
  → Создаётся сессия
  → AuthGate опрашивает /login/code-check.php каждые 2.5 сек
...
Пользователь открывает приложение
  → AuthGate находит токен в localStorage
  → GET /login/session-validate.php?token=xxx
  → Сессия валидна → приложение открывается мгновенно
